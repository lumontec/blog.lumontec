...
const siteUrl = 'https://lumon.tec'

const feed = new Feed({
  title: 'blog.lumontec',
  description: 'Just some dips in my tech trips',
  id: siteUrl,
  link: siteUrl,
  language: 'en',
  image: `${siteUrl}/favicon-32x32.png`,
  favicon: `${siteUrl}/favicon.ico`,
  copyright: `All rights reserved ${new Date().getFullYear()}, @lumontec`,
  feedLinks: {
  ...
  author: {
    ...
    link: 'https://github.com/@lumontec',
  },
})
...
getAllPosts().forEach(({ link, module: { meta, default: Content } }) => {
  ...
  const postText = `<p><em>(The post <a href="${siteUrl + link}">${
    meta.title
  }</a> appeared first on <a href="${siteUrl}">blog.lumontec</a>.)</em></p>`
  feed.addItem({
    title: meta.title,
    id: meta.title,
    link,
