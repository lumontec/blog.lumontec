...
These events will be efficiently pushed to userspace by means of the **[bpf ringbuffer](https://www.kernel.org/doc/html/latest/bpf/ringbuf.html)**, a kernel bpf faciltiy of type <code><strong>BPF_MAP_TYPE_RINGBUF</strong></code> and sample code <code>function (int ciao, char *pippo) {}</code>

```c  
function (int ciao, char *pippo) {}
```


* **userspace executable**: this will launch the executable  
